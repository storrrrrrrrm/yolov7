...
def train(hyp, opt, device, tb_writer=None):
    ...
    if rank in [-1, 0]:
        ...
        testloader, _ = create_dataloader(train_path, imgsz, batch_size, gs, opt,
                                            hyp=hyp, augment=False, cache=opt.cache_images, rect=opt.rect, rank=rank,
                                            world_size=opt.world_size, workers=opt.workers,
                                            image_weights=opt.image_weights, quad=opt.quad, prefix=colorstr('val: '),isLane=True)

        # testloader, _ = create_dataloader(test_path, imgsz, batch_size, gs, opt,
        #                                     hyp=hyp, augment=False, cache=opt.cache_images, rect=opt.rect, rank=rank,
        #                                     world_size=opt.world_size, workers=opt.workers,
        #                                     quad = opt.quad,
        #                                     prefix=colorstr('val: '),isLane=True)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, multi_targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # print('not empty train targets:{}'.format(lane_targets[lane_targets!=0]))
            ...
            with amp.autocast(enabled=cuda):
                ...
                if 'loss_ota' not in hyp or hyp['loss_ota'] == 1:
                    ...
                    pos_weight = exponential_decay(epoch,500,100,100) #从200衰减到1,共计100epoch                    
                    if pos_weight > 1:
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                ...
                results, maps, times = test_banqiao.test(data_dict,
    ...
    if rank in [-1, 0]:
        ...
        for m in (last, best) if best.exists() else (last):  # speed, mAP tests
            results, _, _ = test_banqiao.test(opt.data,
                                        batch_size=batch_size * 2,
                                        imgsz=imgsz_test,
                                        conf_thres=0.001,
                                        iou_thres=0.7,
