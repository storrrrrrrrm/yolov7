"""
u have to put this py in same folder with train.py in which torch.save() is called 
"""

import onnx
import torch
def export_onnx(pt_path,onnx_save_path,input_size,dynamic=False):
    ...
    weights = torch.load(pt_path) #model input:1280
    ...
    device = "cpu"
    model = weights['model'].float() #模型转换为fp32
