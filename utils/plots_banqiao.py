...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    # print('not empty targets:{}'.format(targets[targets!=0]))
    ...
    for i, img in enumerate(images):
        ...
        new_img = img.transpose(1, 2, 0)[...,::-1].astype(np.uint8).copy() # must use copy or ascontiguousarray
        ...
        for j,cls in enumerate(cls_names): 
            ...
            new_img[h_idx,w_idx,0] = color[0]
            new_img[h_idx,w_idx,1] = color[1]
            new_img[h_idx,w_idx,2] = color[2]

        # Draw image filename labels
        if paths:
            label = Path(paths[i]).name[:40]  # trim to 40 char
            t_size = cv2.getTextSize(label, 0, fontScale=tl / 3, thickness=tf)[0]
            cv2.putText(new_img, label, (5, t_size[1] + 5), 0, tl / 3, [220, 220, 220], thickness=tf,
                        lineType=cv2.LINE_AA)
            
        save_name = '{}_image{}.png'.format(fname,i)
